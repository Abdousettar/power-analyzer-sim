...
def test_active_power_pure_resistive():
    ...
    t = np.arange(0, 1, 1/fs)
    v = np.sin(2 * np.pi * 50 * t)
    i = np.sin(2 * np.pi * 50 * t)  # aynı faz
    P = calculate_active_power(v, i)
    # Ortalama güç ≈ 0.5 (çünkü sin^2’in ortalaması 0.5)
...
def test_power_factor_cos_phi():
    ...
    t = np.arange(0, 1, 1/fs)
    v = np.sin(2 * np.pi * 50 * t)
    i = np.sin(2 * np.pi * 50 * t + np.pi/6)  # 30° önde
...
def test_phase_detection():
    ...
    t = np.arange(0, 1, 1/fs)
    v = np.sin(2 * np.pi * 50 * t)
    i = np.sin(2 * np.pi * 50 * t + np.pi/4)  # 45° faz farkı
    phi_rad, phi_deg = calculate_phase(v, i, fs)
    assert np.isclose(phi_deg, 45, atol=5)  # birkaç derece tolerans
